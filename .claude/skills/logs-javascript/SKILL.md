...
- `references/javascript.md` - JavaScript web logs installation
...
- When verifying with an automated browser (Playwright, Puppeteer, Selenium), posthog-js's bot filter silently drops every capture while flags and asset loads still succeed. Override navigator.webdriver, the user agent, AND navigator.userAgentData before concluding events do not send. Diagnose with ?\_\_posthog_debug=true ("likely bot" in the console).
